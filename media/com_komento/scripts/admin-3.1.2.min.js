...
})}})}(c)};d(),b.resolveWith(d)};KTVendors.module("ui/widget",a)}()}),KTVendors.installer("Komento","definitions",function(a){a.module(["komento/shared/elements","komento/admin/admin","komento/admin/api/table","komento/admin/comments/actions","komento/admin/db/db","komento/admin/integrations/smileys","komento/admin/maintenance/database","komento/admin/maintenance/maintenance","komento/admin/migrators/actions","komento/admin/migrators/common","komento/admin/migrators/custom","komento/admin/migrators/progress","komento/admin/reports/actions"])
...
}),b.resolve()}),Komento.require().script("shared/elements","admin/api/table").library("dialog","select2").done(function(a){var b=a("[data-kt-table]");a("[data-table-grid-filter]").select2({theme:"backend",width:"resolve",minimumResultsForSearch:1/0}),a("[data-table-grid-filter]").on("select2:open",function(){a("body").addClass("has-select2-dropdown")}),a("[data-table-grid-filter]").on("select2:close",function(){a("body").removeClass("has-select2-dropdown")}),Komento.compareVersion=function(a,b){for(var c=0,d=a.split("."),e=b.split("."),f=Math.max(d.length,e.length),g=0;f>g;g++){var h=g<d.length?parseInt(d[g],10):0,i=g<e.length?parseInt(e[g],10):0;
if(isNaN(h)&&(h=0),isNaN(i)&&(i=0),h!=i){c=h>i?1:-1;break}}return c},b.length>0&&b.implement(Komento.Controller.Api.Table)}),Komento.module("admin/api/table",function(){var b=this;Komento.require().done(function(){Komento.Controller("Api.Table",{defaultOptions:{"{task}":"[data-kt-table-task]","{search}":"[data-kt-table-search]","{searchInput}":"[data-kt-table-search-input]","{resetSearch}":"[data-kt-table-search-reset]","{checkAll}":"[data-kt-table-checkall]","{checkboxes}":"input[type=checkbox][data-kt-table-id]","{stateButton}":"[data-kt-table-publishing]","{row}":"tr","{boxChecked}":"[data-kt-table-boxchecked]","{filters}":"[data-kt-table-filter]","{paginationLink}":"[data-kt-pagination-link]","{limitstart}":"[data-kt-limitstart-value]"}},function(a){return{init:function(){},"{paginationLink} click":function(b,c){c.preventDefault(),c.stopPropagation();
var d=b.data("limitstart");b.hasClass("active")||b.hasClass("disabled")||(a.limitstart().val(d),a.submitForm())},"{stateButton} click":function(b){var d=b.parents(a.row.selector),e=b.data("task");a.selectRow(d),a.setTask(e),a.submitForm()},"{filters} change":function(){a.setTask(""),a.submitForm()},"{search} click":function(){a.submitForm()},"{resetSearch} click":function(){a.searchInput().val(""),a.submitForm()},submitForm:function(){a.element.submit()},setTask:function(b){a.task().val(b)},setOrdering:function(b){a.ordering().val(b)
},setDirection:function(b){a.direction().val(b)},updateBoxChecked:function(){var b=a.checkboxes(":checked").length;a.boxChecked().val(b)},toggleSelectRow:function(a){var b=a.find("input[name=cid\\[\\]]"),c=1==b.prop("checked");return c?(b.prop("checked",!1),void 0):(b.prop("checked",!0),void 0)},selectRow:function(a){var b=a.find("input[name=cid\\[\\]]");0==b.length&&(b=a.find("input[name=cid]")),b.prop("checked",!0)},"{checkboxes} click":function(a,b){b.stopPropagation()},"{row} click":function(b){var d=b.find(a.checkboxes.selector);
d.prop("checked",!d.is(":checked")).trigger("change")},"{checkboxes} change":function(b){{var d=b.is(":checked"),e=b.closest(a.row.selector);a.checkboxes().is(":checked").length}a.updateBoxChecked(),e.toggleClass("is-checked",d)},"{checkAll} change":function(b){a.checkboxes().prop("checked",b.is(":checked")).trigger("change");b.is(":checked")?a.checkboxes().length:0;a.updateBoxChecked()}}}),b.resolve()})}),Komento.module("admin.comment.actions",function(a){var b=this;Komento.require().library("dialog").script("komento.common","admin.language").done(function(){var c={};
"1.5"==Komento.options.jversion?(c.published="images/tick.png",c.unpublished="images/publish_x.png"):(c.published="templates/bluestork/images/admin/tick.png",c.unpublished="templates/bluestork/images/admin/publish_x.png"),Komento.actions={loadReplies:function(b){var c=a(".kmt-row").length;Komento.ajax("admin.views.comments.loadreplies",{parentId:b,startCount:c},{success:function(c){a("#kmt-"+b).after(c).find(".linked-cell").text("-"),a(".kmt-row").each(function(a,b){var c=a%2;b.removeClass("row1","row0").addClass("row"+c)
}),Komento.options.jversion<"3.0"&&a("#toggle").attr("onClick","checkAll("+a(".kmt-row").length+");")}})},submit:function(b,c){0!=a(".foundryDialog").length&&a(".foundryDialog").controller().close(),Komento.actions.affectchild=c;var d=new Array,e=new Array;a('input[type="checkbox"]:checked').each(function(c,f){if(""!=f.value){d.push(f.value),e.push(a("#kmt-"+f.value));var g;g="unstick"==b||"stick"==b?"sticked":"published",a("#kmt-"+f.value).find("."+g+"-cell a img").attr("src",Komento.options.spinner)}}),Komento.ajax("admin.views.comments."+b,{ids:d,affectchild:c},{success:function(){var c=[],d=[];
...
},countArticle:function(a){return void 0===a?b.count().html():b.count().html(a)},populateComplete:function(){b.status().html(a.language("COM_KOMENTO_SETTINGS_DATABASE_POPULATE_DEPTH_STATUS_COMPLETED")),b.start().enabled(!0)}}}),Komento.Controller("Database.FixStructure",{defaultOptions:{"{component}":".componentSelection","{article}":".articleSelection","{start}":".start","{fixstatus}":".fixStatus","{total}":".total","{count}":".count","{statusWrapper}":"[data-status-wrapper]","{totalWrapper}":"[data-total-wrapper]","{countWrapper}":"[data-count-wrapper]"}},function(b){return{init:function(){},"{component} change":function(c){var d=c.val();
...
b.find("input").val(""),a.addRow().before(b)},"{delete} click":function(b){var c=b.parent("[data-smiley]");a.smiley().length>1?c.remove():c.find("input").val("")}}}),b.resolve()}),Komento.module("admin/maintenance/database",function(){var b=this;Komento.Controller("Maintenance.Execute",{defaultOptions:{"{container}":"[data-maintenance-database]","{start}":"[data-start]","{progress}":"[data-progress]","{progressBox}":"[data-progress-box]","{progressBar}":"[data-progress-bar]","{progressPercentage}":"[data-progress-percentage]","{errorBox}":"[data-error]","{successBox}":"[data-success]"}},function(a){return{init:function(){},counter:0,versions:[],"{start} click":function(b){b.hide(),a.progress().show(),a.process()
},process:function(){a.getStats().done(function(b){a.versions=b,a.execute()})},getStats:function(){return Komento.ajax("admin/controllers/maintenance/getDatabaseStats")},execute:function(){return void 0===a.versions[a.counter]?a.completed():(a.successBox().hide(),Komento.ajax("admin/controllers/maintenance/synchronizeDatabase",{version:a.versions[a.counter]}).fail(function(b){var c=a.errorBox();c.show(),c.append('<div class="alert alert-danger">'+b+"</div>")}).always(function(){a.counter++;var b=Math.floor(a.counter/a.versions.length*100)+"%";
a.progressBar().css("width",b),a.progressPercentage().text(b),a.execute()}),void 0)},completed:function(){a.progressBar().css("width","100%"),a.progressPercentage().text("100%"),a.progressBox().removeClass("progress-info").addClass("progress-success"),a.successBox().delay(1e3).fadeIn(400)}}}),b.resolve()}),Komento.module("admin/maintenance/maintenance",function(){var b=this;Komento.Controller("Maintenance.Execute",{defaultOptions:{"{row}":"[data-row]"}},function(a){return{init:function(){a.runscript()},counter:0,success:0,fail:0,runscript:function(){var b=a.row().eq(a.counter);
if(0===b.length)return a.completed();var c=b.data("key");Komento.ajax("admin/controllers/maintenance/runscript",{key:c}).done(function(){a.setStatus(b,1),a.success++}).fail(function(){a.setStatus(b,0),a.fail++}).always(function(){a.counter++,a.runscript()})},completed:function(){a.fail<1&&(window.location="index.php?option=com_komento&view=maintenance&success="+a.success)},setStatus:function(a,b){var c=a.find("[data-status]"),d=a.find("[data-icon]"),e=["label-danger","label-success","label-warning"],f=["fa-exclamation-triangle","fa-check","fa-wrench"];
for(i=0;3>i;i++)c.toggleClass(e[i],b==i),d.toggleClass(f[i],b==i)}}}),b.resolve()}),Komento.module("migrator.actions",function(a){var b=this;Komento.require().script("admin.language","komento.common").done(function(){Komento.Controller("Migrator.Actions",{defaults:{"{migrateButton}":".migrateButton","{deleteButton}":".deleteButton","{migrateSettingsButton}":".migrateSettingsButton"}},function(b){return{init:function(){b.progress=b.element.find(".migratorProgress"),b.migrator=b.element.find(".migratorTable")},"{migrateButton} click":function(a){a.checkClick()&&b.migrateStart()
...
}}),void 0)}}}),b.resolve()}),Komento.module("admin/migrators/custom",function(a){var b=this;Komento.Controller("Migrator.Custom",{defaults:{"{migrateTable}":"[data-custom-table]","{componentFilter}":"[data-custom-component-filter]","{tableColumns}":"[data-table-columns]","{cycleAmount}":"[data-migrate-cycle]"}},function(b){return{init:function(){b.loadTableColumns()},"{migrateTable} change":function(){b.loadTableColumns()},loadTableColumns:function(){var c=b.migrateTable().val();Komento.ajax("admin/views/migrators/getColumns",{tableName:c}).done(function(c){b.tableColumns().each(function(b,d){var e=c;
a(d).data("required")||(e='<option value="notavailable">none</option>'+c),a(d).html(e)})})},getData:function(){var c={};return c.table=b.migrateTable().val(),b.tableColumns().each(function(b,d){var e=a(d).attr("id").slice(15);c[e]=a(d).val()}),c.componentFilter=b.componentFilter().val(),c.cycle=b.cycleAmount().val(),c}}}),b.resolve()}),Komento.module("migrator.progress",function(){var b=this;Komento.require().script("admin.language","komento.common").done(function(){Komento.Controller("Migrator.Progress",{defaults:{"{progressBar}":".progressBar","{progressStatus}":".progressStatus","{progressPercentage}":".progressPercentage","{logList}":".logList","{clearLog}":".clearLog","{totalComments}":".totalComments","{totalPosts}":".totalPosts","{migratedComments}":".migratedComments"}},function(a){return{init:function(){},"{clearLog} click":function(){a.logList().html("")
...
if(d){for(var e=0;!0;e++){var f=c["cb"+e];if(!f)break;f.checked=!1}d.checked=!0,c.boxchecked.value=1,submitbutton(b)}return!1},b.resolve()})})});