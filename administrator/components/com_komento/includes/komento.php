	...
	public static function commentify($component, &$article, $options = array())
	{
		...
		if ($application->isListingView()) {
			...
			if ($needRating) {
				...
				KT::initialize();
			} elseif (!$needRating && ($config->get('layout_frontpage_comment') || $config->get('layout_frontpage_hits') || $config->get('layout_frontpage_preview'))) {
				// Load frontend styles without javascript
