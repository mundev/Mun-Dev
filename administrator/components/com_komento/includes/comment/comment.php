...
class KomentoComment extends KomentoBase
{
	...
	public function getContent($truncate = false, $insertEllipses = false)
	{
		...
		if (!isset($result[$key])) {
			...
			if ($truncate) {
				...
				$contents = substr(strip_tags($contents), 0, $truncate);
	...
	{
		$cleantalkRejection = $this->config->get('antispam_cleantalk_rejection_type');

		// CleanTalk antispam detection
		if (KT::cleantalk()->isEnabled() && $cleantalkRejection == 'high') {
			if ($this->isCleantalkSpam()) {
	 ...
	 * @return
	 */
	public function isCleantalkSpam()
	{
