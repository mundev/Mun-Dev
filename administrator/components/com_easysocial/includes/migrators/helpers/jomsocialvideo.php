...
class SocialMigratorHelperJomsocialVideo
{
	...
	private function processMentions(&$commentObj)
	{
	 ...
	 * @access private
	 */
	private function processCommentAttachment($jsComment, $esComment)
	{
		$config = ES::config();
		$db = ES::db();
		...
		if ($jsPhotoId) {
			...
			if ($jsPhoto->id) {
				...
				if (!JFolder::exists($dest)) {
					...
					$indexFile = $dest . '/index.html';
					JFile::write($indexFile, '<html><body bgcolor="#FFFFFF"></body></html>');
				}
	...
	private function processComments()
	{
		...
		foreach($jsVideoComments as $jsVideoComment) {
			...
			if ($jsCommentAttachmentId) {
				$this->processCommentAttachment($jsVideoComment, $esComment);
			}
