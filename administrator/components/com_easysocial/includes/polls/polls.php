...
class SocialPolls extends EasySocial
{
	...
	/**
	 * Notify the cluster when new polls is created
	 *
	 * @since	2.1
	 * @access	public
	 */
	public function notifyCluster($cluster, $poll, $streamItem)
	{
		// Get the actor
		$actor = ES::user($streamItem->actor_id);

		// Get a list of cluster members
		$model = ES::model($cluster->getTypePlural());
		$targets = $model->getMembers($cluster->id, array('exclude' => $actor->id, 'state' => SOCIAL_STATE_PUBLISHED, 'users' => true));
		...
		$mailParams[$cluster->getType()] = $cluster->getName();
		$mailParams[$cluster->getType() . 'Link'] = $cluster->getPermalink(true, true);
		$mailParams['permalink'] = $permalink;
		$mailParams['title'] = 'APP_' . strtoupper($cluster->getType()) . '_STORY_EMAILS_NEW_POLLS_IN_' . strtoupper($cluster->getType());
		$mailParams['template'] = 'apps/' . $cluster->getType() . '/polls/new.polls';

		// Prepare the system notification params
		$systemParams = array();
		$systemParams['context_type'] = $cluster->getTypePlural();
		$systemParams['title'] = JText::sprintf('APP_' . strtoupper($cluster->getType()) . '_STORY_POLLS_CREATED_IN_' . strtoupper($cluster->getType()), $actor->getName(), $cluster->getName());
		...
		$state = ES::notify($cluster->getTypePlural() . '.polls.create', $targets, $mailParams, $systemParams, $cluster->notification);
	}
