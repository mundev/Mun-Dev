...
class EasyBlogInstantArticles extends EasyBlog
{
	...
	public static function processInstantContent(&$content = '')
	{
		...
		preg_match_all('#<strong>(.*)<\/strong>#U', $content, $matches);
		// preg_match_all('#<h2><strong>(.*)<\/strong><\/h2>#', $content, $matches);

		foreach ($matches[0] as $key => $row) {
			$content = str_replace($row, $matches[1][$key], $content);
		}
	...
	public static function clean(&$content = '', $debug = false)
	{
		...
		$content = preg_replace('#(<br\ ?\/?>)#iUum', '', $content);
